...
def test_the_prompt_ends_on_the_writers_last_character() -> None:
    ...
    assert render_prompt("the okiya was").endswith("the okiya was")
...
async def test_the_rendered_prompt_is_what_gets_sent(tmp_path: Path) -> None:
    ...
    await collect(llm_service(tmp_path, handler).stream("p/m", "the okiya was"))
    sent = seen["messages"][0]["content"]
    assert sent == render_prompt("the okiya was")
    assert sent.endswith("the okiya was")
...
def test_generation_streams_deltas_then_done(llm_client) -> None:
    ...
    response = llm_client.post(
        "/api/llm/generate", json={"model": "p/m", "prompt": "the okiya was"}
    )
    assert response.status_code == 200
